    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[999] w-[85vw] max-w-xs rounded-3xl overflow-hidden"
        ...
        <div className="px-6 py-3 text-center">
          ...
          <p className="text-sm text-neutral-500 leading-relaxed">This comment will be permanently removed.</p>
        </div>
        <div className="flex gap-2.5 px-5 pb-5 pt-2">
          <button onClick={onCancel} className="flex-1 py-3 rounded-2xl text-sm font-bold text-white"
            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>Cancel</button>
          <button onClick={onConfirm} className="flex-1 py-3 rounded-2xl text-sm font-bold text-white"
...
export function PostCard({ post, currentUserId }: PostCardProps) {
  ...
  const handleSave = async () => {
    ...
    setSaved(!wasSaved);
    try {
      if (wasSaved) { await api.delete(`/posts/${post.id}/save`); showNotif("Removed from saved"); }
      else { await api.post(`/posts/${post.id}/save`); showNotif("Post saved!"); }
    } catch { setSaved(wasSaved); }
    finally { setIsSaving(false); }
  };

  // Desktop: open as overlay on home page
  const handleImageClickDesktop = (e: React.MouseEvent) => {
    e.preventDefault();
    router.push(`?postId=${post.id}&liked=${liked}&likeCount=${likeCount}&saved=${saved}`, { scroll: false });
  };

  const isLong = post.caption.length > 100;
  const displayCaption = !showFull && isLong ? post.caption.slice(0, 100) + "..." : post.caption;
  ...
  return (
    <>
      ...
      {deleteTarget !== null && (
        <DeleteDialog
          onConfirm={() => {
            window.dispatchEvent(new CustomEvent("confirm-delete-comment", { detail: deleteTarget }));
      ...
      <main className="flex flex-col w-full">
        ...
        <Link href={`/post/${post.id}?liked=${liked}&likeCount=${likeCount}&saved=${saved}`} className="md:hidden">
        ...
        <div className="relative w-full rounded-3xl aspect-square overflow-hidden cursor-pointer hidden md:block"
          onClick={handleImageClickDesktop}>
          <Image src={post.imageUrl} alt={post.caption} fill className="object-cover" sizes="(min-width: 768px) 600px, 100vw" />
        </div>
        ...
        <div className="flex items-center justify-between px-4 pt-3 pb-1">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1">
              <button onClick={handleLike} disabled={isLiking} className="text-white">
          ...
          <button onClick={handleSave} disabled={isSaving}
            ...
            {saved ? <IoBookmark className="size-7 text-[#7c5cfc]" /> : <IoBookmarkOutline className="size-7" />}
          </button>
        </div>

        {/* Caption */}
        <div className="px-4 py-2 pb-4">
          <p className="text-sm text-white font-bold mb-2">{post.author.name}</p>
          <p className="text-sm text-neutral-50">
            {displayCaption}
            {isLong && (
              <button onClick={() => setShowFull((p) => !p)}
                className="flex text-[var(--primary-200)] font-semibold mt-2">
      ...
      {showLikes && <LikesSheet postId={post.id} likeCount={likeCount} onClose={() => setShowLikes(false)} />}
