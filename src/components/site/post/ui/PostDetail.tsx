...
import { usePostDetail, type PostDetailData } from "@/hooks/post/usePostDetail";
...
export function PostDetail({
  ...
  const {
    liked, likeCount, saved, toast,
    isPendingLike, isPendingSave, isDeletingPost,
    handleLike, handleSave, handleDeletePost,
  } = usePostActions({
    postId,
  ...
  const isOwner = currentUserId !== undefined && post.author.id === currentUserId;
  ...
  return (
    <>
      ...
      {showLikes && (
        <LikesSheet postId={postId} likeCount={likeCount} onClose={() => setShowLikes(false)} />
      )}
      ...
      {deleteCommentId !== null && (
        <DeleteDialog
          ...
          onConfirm={() => {
            window.dispatchEvent(new CustomEvent("confirm-delete-comment", { detail: deleteCommentId }));
      ...
      <div className="flex flex-col min-h-screen bg-[#0e0e13] text-white md:hidden">
        <div className="flex items-center justify-between px-4 py-4">
          ...
          {isOwner && <PostMenuButton onDeleteClick={() => setShowDeletePost(true)} size="md" />}
        </div>
      ...
      >
        ...
        >
          <div className="relative w-[520px] shrink-0 bg-black">
            <Image src={post.imageUrl} alt={post.caption} fill className="object-cover" sizes="520px" />
          </div>
          ...
          <div className="flex flex-col flex-1 min-w-0 bg-[#0e0e13]">
            <div className="flex items-center justify-between px-4 py-3 shrink-0 border-b border-[rgba(255,255,255,0.06)]">
              ...
              <div className="flex items-center gap-1">
                {isOwner && <PostMenuButton onDeleteClick={() => setShowDeletePost(true)} size="sm" />}
                <button
                  onClick={handleClose}
            ...
            <div className="px-4 py-3 shrink-0">
              <span className="text-sm font-bold text-white mr-2">{post.author.name}</span>
              <span className="text-sm text-neutral-200">{post.caption}</span>
            </div>
