...
import {
  ...
  IoArrowBack, IoCheckmark, IoTrashOutline, IoCloseOutline,
  IoEllipsisVertical,
...
function Toast({ message, show }: { message: string; show: boolean }) {
  return (
    ...
    >
      <div className="flex items-center justify-center size-5 rounded-full shrink-0"
...
function DeleteCommentDialog({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    <>
      <div className="fixed inset-0 z-[998] bg-black/70 backdrop-blur-sm" onClick={onCancel} />
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[999] w-[85vw] max-w-xs rounded-3xl overflow-hidden"
        style={{ background: "linear-gradient(160deg, rgba(20,20,32,0.99) 0%, rgba(10,10,18,1) 100%)", border: "1px solid rgba(255,255,255,0.07)", boxShadow: "0 24px 60px rgba(0,0,0,0.7)" }}>
        ...
        <div className="px-6 py-3 text-center">
          ...
          <p className="text-sm text-neutral-500 leading-relaxed">This comment will be permanently removed and cannot be undone.</p>
        </div>
        <div className="flex gap-2.5 px-5 pb-5 pt-2">
          <button onClick={onCancel} className="flex-1 py-3 rounded-2xl text-sm font-bold text-white transition-all active:scale-95"
            style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>Cancel</button>
          <button onClick={onConfirm} className="flex-1 py-3 rounded-2xl text-sm font-bold text-white transition-all active:scale-95"
            style={{ background: "linear-gradient(135deg, #f87171 0%, #ef4444 100%)", boxShadow: "0 4px 16px rgba(239,68,68,0.35)" }}>Delete</button>
        </div>
      </div>
    </>
  );
...
function DeletePostDialog({ onConfirm, onCancel, loading }: { onConfirm: () => void; onCancel: () => void; loading: boolean }) {
  return (
    <>
      <div className="fixed inset-0 z-[998] bg-black/70 backdrop-blur-sm" onClick={onCancel} />
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[999] w-[85vw] max-w-xs rounded-3xl overflow-hidden"
        ...
        <div className="px-6 py-3 text-center">
          ...
          <p className="text-sm text-neutral-500 leading-relaxed">This post will be permanently removed and cannot be undone.</p>
        </div>
        <div className="flex gap-2.5 px-5 pb-5 pt-2">
          ...
          <button onClick={onConfirm} disabled={loading}
            ...
            {loading ? <div className="size-4 rounded-full border-2 border-white border-t-transparent animate-spin" /> : "Delete"}
          </button>
        </div>
      </div>
    </>
...
export function PostDetail({ postId, currentUserId, initialLiked, initialLikeCount, initialSaved }: { postId: number; currentUserId?: number; initialLiked?: boolean; initialLikeCount?: number; initialSaved?: boolean }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setPost(d);
      // NEVER override liked/likeCount/saved from API — always use initialLiked passed from page
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      // Store updated like state so profile grid can read it
      localStorage.setItem(`post_like_${postId}`, JSON.stringify({ liked: !wasLiked, likeCount: newLikeCount }));
    } catch {
      setLiked(wasLiked);
  ...
  const handleSave = async () => {
    ...
    setSaved(!wasSaved);
    try {
      if (wasSaved) { await api.delete(`/posts/${postId}/save`); showNotif("Removed from saved"); }
      else          { await api.post(`/posts/${postId}/save`);   showNotif("Post saved!"); }
  ...
  if (!post) return (
    <div className="flex items-center justify-center min-h-screen bg-[#0e0e13] text-neutral-400 text-sm">Post not found</div>
  );
  ...
  return (
    <>
      ...
      {showLikes && <LikesSheet postId={postId} likeCount={likeCount} onClose={() => setShowLikes(false)} />}

      {deleteTarget !== null && (
        <DeleteCommentDialog
          onConfirm={() => {
            const event = new CustomEvent("confirm-delete-comment", { detail: deleteTarget });
      ...
      <div className="flex flex-col min-h-screen bg-[#0e0e13] text-white md:flex-row md:h-screen md:overflow-hidden">
        ...
        <div className="flex flex-col md:flex-1 md:overflow-y-auto">
          ...
          {/* Image + 3-dot for owner */}
          <div className="relative w-full aspect-square md:rounded-xl overflow-hidden">
            <Image src={post.imageUrl} alt={post.caption} fill className="object-cover" sizes="100vw" />
            {currentUserId !== undefined && currentUserId === post.author.id && (
              <div className="absolute top-3 right-3 z-10">
                <button
                  ...
                  style={{ background: "rgba(10,10,18,0.65)", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(8px)" }}
                >
                ...
                {showPostMenu && (
                  <>
                    ...
                    >
                      <button
                        onClick={() => { setShowPostMenu(false); setShowDeletePost(true); }}
          ...
          <div className="flex items-center justify-between px-4 pt-3 pb-1">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setShowLikes(true)} className="text-sm text-white">{likeCount}</button>
              </div>
              <button onClick={() => setShowComments(true)} className="flex items-center gap-1 text-white">
                <IoChatbubbleOutline className="size-6" />
                <span className="text-sm">{commentCount}</span>
              </button>
            </div>
            <button onClick={handleSave} className="relative text-white transition-transform active:scale-90">
              {saved
          ...
          <button onClick={() => setShowComments(true)}
            className="md:hidden mx-4 mb-28 text-left text-sm text-neutral-500 rounded-2xl px-4 py-6"
        ...
        {/* Desktop right panel */}
        <div className="hidden md:flex md:flex-col md:w-[360px] md:border-l md:border-[rgba(126,145,183,0.15)]">
          <CommentSection postId={postId} currentUserId={currentUserId} inline
